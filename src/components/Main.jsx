...
const Main = () => {
    ...
    const getUpcomingLessons = () => {
        api.get('https://www.fluentenglish.site/schedule/schedule/').then(
            (res) => {
                let data = res.data;
                console.log(data);
                setTables(data);
                ...
                if (data.length > 0) {
                    if (checkRemainingTime(data[0])) setUpcomingLesson(data[0]);
                }
            }
        );
    };
    ...
    const checkRemainingTime = (lesson) => {
        const lessonTime = new Date(`${lesson.date} ${lesson.time}`);
        const remainingTime = lessonTime.getTime() - new Date().getTime();
                                ...
                                >
                                    <div>
                                        <Typography
                                            ...
                                            color="#e29578"
                                        >
                                            Last note from lesson -{' '}
                                            {notes[notes.length - 1]?.lesson}
                                        </Typography>
                                        <Typography
                                            variant="body2"
                                            component="div"
