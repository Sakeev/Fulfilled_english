...
export default function Sidebar({ children }) {
  ...
  if (!isTeacher) {
    filteredSections = sections.filter(
      (section) => section.title !== "Gradebook"
    );
  }
  return (
      ...
      <div className={styles.sidebar}>
        <div className={styles.content}>
          ...
          <ul className={styles.sections}>
            {filteredSections.map(({ title, icon, activeIcon, alt, route }) => (
              <NavLink to={route} key={route}>
                <li>
                  <img src={icon} alt={alt} />
                  <img className={styles.active} src={activeIcon} alt={alt} />
                  <span>{title}</span>
                </li>
              </NavLink>
            ))}
            <li onClick={logout}>
              <img src={logoutSection.icon} alt={logoutSection.alt} />
              <img
